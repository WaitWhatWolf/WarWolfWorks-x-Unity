...
namespace WarWolfWorks.Utility.Coloring
{
    ...
    /// </summary>
    [AddComponentMenu(IN_ASSETMENU_WARWOLFWORKS + IN_ASSETMENU_UTILITY + nameof(ColorManager))]
    public sealed class ColorManager : MonoBehaviour
    {
        /// <summary>
        /// The base color that the color manager will default to.
        /// </summary>
        [FormerlySerializedAs("s_BaseColor"), SerializeField]
        ...
        private void Awake()
        {
            for (int i = 0; i < IN_COLORMANAGER_LAYER_MAX; i++)
            {
        ...
        private void Update()
        {
            ...
            {
                AdvancedDebug.LogWarning($"Couldn't Apply color, {e}", DEBUG_LAYER_EXCEPTIONS_INDEX);
                return;
            }
        }
        ...
        {
            if (original > IN_COLORMANAGER_LAYER_MAX)
            {
                AdvancedDebug.LogWarningFormat("Layer {0} was set too high. Changing to layer {1}.", 0, original, IN_COLORMANAGER_LAYER_MAX);
                original = IN_COLORMANAGER_LAYER_MAX;
            }
            else if (original < 0)
            {
        ...
        private Dictionary<int, List<CMColor>> Colors = new Dictionary<int, List<CMColor>>(IN_COLORMANAGER_LAYER_MAX);
